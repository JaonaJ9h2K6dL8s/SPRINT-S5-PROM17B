...
public class FrontController extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (mapping != null) {
            try {
                ...
                // Si c'est une méthode JSON, retourner une erreur JSON
                if (mapping != null && isJsonMethod(mapping.method)) {
    ...
    // Méthode pour vérifier si une méthode est annotée avec @Json
    private boolean isJsonMethod(Method method) {
        try {
            // Vérifie d'abord si l'annotation existe dans le package
            Class<?> jsonAnnotationClass = Class.forName("com.monframework.Json");
            return method.getAnnotation((Class<java.lang.annotation.Annotation>) jsonAnnotationClass) != null;
        } catch (ClassNotFoundException e) {
            System.out.println("⚠️ Annotation @Json non trouvée");
            return false;
        }
    }
    ...
    // NOUVELLE MÉTHODE POUR GÉRER LES RÉSULTATS JSON (sans Jackson)
    private void handleJsonResult(Object result, HttpServletResponse response)
            throws ServletException, IOException {
        
        System.out.println("📡 Retour JSON détecté");
        
        // Configurer la réponse HTTP pour JSON
        ...
        try {
            ...
            if (result instanceof Map) {
                // Si c'est déjà un Map, on le convertit en JSON manuellement
                jsonResponse = mapToJson((Map<?, ?>) result);
            } else if (result instanceof java.util.List) {
                // Si c'est une liste, on crée la structure standardisée avec "result" et "count"
                java.util.List<?> list = (java.util.List<?>) result;
                Map<String, Object> jsonStructure = new HashMap<>();
                jsonStructure.put("status", "success");
                jsonStructure.put("code", 200);
                jsonStructure.put("result", list);
                jsonStructure.put("count", list.size());
                jsonResponse = mapToJson(jsonStructure);
            } else {
                // Pour un objet simple, on utilise "data"
                Map<String, Object> jsonStructure = new HashMap<>();
                jsonStructure.put("status", "success");
                jsonStructure.put("code", 200);
                jsonStructure.put("data", result);
                jsonResponse = mapToJson(jsonStructure);
            }
            ...
            System.out.println("❌ Erreur lors de la génération JSON: " + e.getMessage());
            e.printStackTrace();
    ...
    // Méthode pour gérer les erreurs en JSON
    private void handleJsonError(Exception e, HttpServletResponse response) throws IOException {
        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");
        ...
        response.getWriter().write(mapToJson(errorResponse));
    }
    
    // Méthodes utilitaires pour convertir en JSON manuellement
    ...
    private String valueToJson(Object value) {
        if (value == null) {
            ...
            // Convertir tableau en liste JSON
            StringBuilder json = new StringBuilder("[");
            ...
            // Pour d'autres objets, les convertir en String
